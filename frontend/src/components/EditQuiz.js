...
export default function EditQuiz() {
  ...
  // Hook for questions to be removed (stores IDs of questions deleted from original set)
  const [removedQuestionIds, setRemovedQuestionIds] = useState([]);
  
  // Note: "toBeAdded" questions are derived from the main 'questions' state 
  // by filtering those without a backend '_id'.
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if (!profileRes.data.user || profileRes.data.user.userType !== "admin") {
          alert("Only admins can access Edit Quiz.");
          window.location.href = `${window.location.origin}/auth`;
          return;
        }
        ...
        if (quizId) {
            ...
            const { quizDetails: qd, questions: qs } = quizRes.data;

            // Parse ISO dates (2025-12-10T17:00) into Date and Time parts
            let sDate = "", sTime = "", rDate = "", rTime = "";
            
            if (qd.startTime && qd.startTime.includes("T")) {
                [sDate, sTime] = qd.startTime.split("T");
            }
            if (qd.resultTime && qd.resultTime.includes("T")) {
                [rDate, rTime] = qd.resultTime.split("T");
            }

            setQuizDetails({
                quizName: qd.quizName || "",
                quizId: qd.quizId || "",
                subject: qd.subject || "",
                description: qd.description || "",
                adminIds: Array.isArray(qd.adminIds) ? qd.adminIds.join(", ") : "",
                startDate: sDate,
                startTime: sTime,
                resultDate: rDate,
                resultTime: rTime,
                durationMinutes: qd.durationMinutes || 0,
                totalMarks: qd.totalMarks || 0,
                negativeMarkPerQuestion: qd.negativeMarkPerQuestion || 0,
                maxAttempts: qd.maxAttempts || 1,
                visibility: qd.visibility || "public",
            });

            // Map questions to ensure they have an internal ID for React keys if needed, 
            // though _id acts as a good key.
            setQuestions(qs.map(q => ({
                ...q,
                // Ensure options is array
                options: q.options || [],
                // Ensure integer handling
                correctInteger: q.correctInteger || "",
                // UI expects correctIndex as number
        ...
        console.log(err);
        alert("Error loading quiz data or unauthorized.");
  ...
  const removeQuestion = (index) => {
    ...
    // If it has a backend _id, add to removedQuestionIds
    if (questionToRemove._id) {
        setRemovedQuestionIds(prev => [...prev, questionToRemove._id]);
    }
  ...
  const addSelectedPYQs = () => {
    ...
    const newQuestions = selected.map(q => {
      // We strip the _id so they are treated as NEW questions for this quiz
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      adminIds: quizDetails.adminIds.split(",").map(item => item.trim()),
    ...
    try {
        ...
        setQuestions(prev =>
          prev.map(q => {
            if (q._id && typeof q.askedIn === "number") {
              return { ...q, askedIn: q.askedIn - 1 };
            }
            return q; // return unchanged object
          })
        );
  ...
  if (loadingData) return <div className="loading-state"><div className="spinner"></div></div>;
        ...
        <div className="main-content-wrapper">
          <header className="page-header">
            <div className="header-left">
              <h1>Edit Quiz</h1>
              <p>Modify quiz settings and questions.</p>
            </div>
            <div className="header-right">
              {/* Delete Quiz Button */}
              <button className="header-btn delete-quiz-btn" onClick={handleDeleteQuiz} type="button">
          ...
          <form onSubmit={handleSubmit} className="create-form">
              ...
              <div className="panel left-panel">
                ...
                <div className="form-scroll-area">
                  <div className="form-grid">
                    ...
                    <div className="form-group"><label>Quiz ID</label><input name="quizId" value={quizDetails.quizId} onChange={handleDetailChange} placeholder="e.g. PY101" disabled/></div>
              ...
              <div className="panel right-panel">
                ...
                <div className="questions-list">
                  {questions.length === 0 ? (
                    ...
                    questions.map((q, idx) => (
                      <div key={idx} className={`question-card ${!q._id ? 'new-question' : ''}`}>
                        <div className="q-card-header">
                          <span className="q-number">Q{idx + 1}</span>
                          {!q._id && <span className="new-badge">NEW</span>}
            ...
            <div className="create-action-bar">
               <button type="submit" className="submit-btn-large"><Save size={20} /> Update Quiz</button>
            </div>
          </form>
        </div>
      </div>
...
.delete-quiz-btn { background: #fee2e2; color: #ef4444; border: 1px solid #fecaca; }
.delete-quiz-btn:hover { background: #fecaca; }
...
.new-question { border: 1px dashed #a855f7; background: #faf5ff; }
.new-badge { background: #a855f7; color: white; font-size: 10px; padding: 2px 6px; border-radius: 4px; font-weight: 700; margin-right: 6px; }
